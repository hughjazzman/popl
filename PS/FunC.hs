...
-- Q6
eval (Address x) env mem = eval x env mem
eval (Contents x) env mem = eval (Apply (Variable "!") [x]) env mem
...
etranslate (Assign (Contents e1) e2) = Assign (etranslate e1) (etranslate e2)
etranslate (Assign e1 e2) = Assign e1 (etranslate e2)
etranslate (Variable x) = let v = maybe_find init_env x in
  case v of
    Just y -> Variable x
    Nothing -> Apply (Variable "!") [Variable x]
etranslate (Apply (Variable f) es) = let v = maybe_find init_env f in
  case v of 
    Just x -> Apply (Variable f) (map etranslate es)
    Nothing -> Apply (etranslate (Variable f)) (map (ref . etranslate) es)
...
-- Q7b
etranslate (Address x) = x
etranslate (Contents x) = Apply (Variable "!") [etranslate x]
etranslate e = e
...
init_env =
  make_env [constant "nil" Nil, 
    ...
    primitive "ref" (\ [a] mem -> 
      let (x, mem') = fresh mem in (Addr x, update mem' x a))]
  where 
    constant x v = (x, v)

module FunMonadCpsJump(main) where
import Parsing
import FunSyntax
import FunParser
import Environment
...
eval (Assign e1 e2) env =
  eval e1 env $> \ (Addr a) -> eval e2 env $> \v -> put a v $> \_ ->
  result Nil
...
get a k = \mem -> k (contents mem a) mem
put a v k = \mem -> k () (update mem a v)
new k = \mem -> let (a,mem') = fresh mem in k a mem'

type Ans = Mem -> (String,(Mem,Env))

obey :: Phrase -> (Mem,Env) -> (String, (Mem,Env))
obey (Calculate exp) (mem,env) =
     (eval exp env (\v -> \mem' -> (print_value v, (mem',env))) mem)
obey (Define def) (mem,env) =
     (let x = def_lhs def in elab def env (\env' -> \mem' -> (print_defn env' x, (mem',env'))) mem)

main = dialog funParser obey (init_mem,init_env)
